...
For development it's often practical to persist query results across page refreshs, especially when working particularly expensive queries that take time to process. You can set a feature flag to persist those queries within the browser. For this, set this environment variable: `VITE_ENABLE_PERSISTENT_QUERIES=true`
